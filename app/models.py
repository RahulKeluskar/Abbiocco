...
class RecipeBookEntry(db.Model):
	id = db.Column(db.Integer,
                                  autoincrement=True,
                                  primary_key=True)
	book_id = db.Column(db.Integer, db.ForeignKey('recipeBook.book_id'))
	recipe_id = db.Column(db.String(64), db.ForeignKey('recipes.recipe_id'))


	def get_recipe(self):
		return Recipe.query.filter_by(recipe_id=self.recipe_id).first()
